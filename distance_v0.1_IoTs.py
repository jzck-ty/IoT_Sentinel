...
#------------------------------------------------------------------
s1=input_data.shape[0] #Whole samples to pick from input device (e.g., F)

with open("data.txt","w") as output:
	output.write(str(input_data))
	output.close



dist=[0]*s2 #distance is an array 5*12 it means 5 samples of 12 packets
for i in range(s2):
	dist[i]=[0]*12
...
for i in range(s1):
		...
		if (result[index][:1]=='1'): #Check if the device has been voted as a match
			...
			for n in range(s2):					
				...
				for m in range(12):
					sum+=dist[n][m]
				ave[n]=sum
			#distance vectors are sumed up for five sample of a divice and are stored in ave and normilazied
			min1=min(ave)
			max1=max(ave)
			...
			if min_local>total_ave:
				min_local=total_ave
				f1=f
    #------------------------------------------
	#print (item[:-4]+" & "+f1[:-4]+" : "+"%f" %(min_local))
	#print("%f" %(min_local),end='\n')
	#------------------------------------------
	if min_global>min_local:
		...
		feature=i+1
		f2=f1
print("Feature No. "+str(feature)+ " from item1[:-4]"+ " matched "+f2[:-4]+" : "+"%f"%(min_global))
